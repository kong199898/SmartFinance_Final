...
class CharityGiftViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editCharityNotification), name: Notification.Name("editCharity"), object: nil)
    }

    @objc func editCharityNotification(_ notification: Notification){
        isEdit = true
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                ...
                createItem(type: "Charity/Gift", amount: Int(AmountPerYearField.text!) ?? 0, frequency: frequency!, StartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }
        }
    }
    ...
    func updateItem(item: TheSpendingGoal, newamount:Int, newfrequency:String, newStartingAge:Int){
        ...
        do{
            ...
            showMessage(alert: "Error", message: "update failed")
        }
    }
}
