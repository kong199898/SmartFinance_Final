...
class HomePurchaseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editHomePurchaseNotification), name: Notification.Name("editHomePurchase"), object: nil)
    }
    
    @objc func editHomePurchaseNotification(_ notification: Notification){
        isEdit = true
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                ...
                createItem(type: "Home Purchase/Upgrade", amount: Int(DownPaymentField.text!) ?? 0, StartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }
        }
    }
