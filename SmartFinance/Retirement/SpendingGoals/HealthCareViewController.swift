...
class HealthCareViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editHealthCareNotification), name: Notification.Name("editHealthCare"), object: nil)
    }
    
    @objc func editHealthCareNotification(_ notification: Notification){
        isEdit = true
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                ...
                createItem(type: "Health Care", amount: Int(AmountPerYearField.text!) ?? 0, frequency: frequency!, StartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }
        }
    }
