...
class VacationViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editVacationNotification), name: Notification.Name("editVacation"), object: nil)
    }
    
    @objc func editVacationNotification(_ notification: Notification){
        isEdit = true
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                ...
                createItem(type: "Vacation", amount: Int(AmountPerYearField.text!) ?? 0, frequency: frequency!, StartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }
        }
    }
