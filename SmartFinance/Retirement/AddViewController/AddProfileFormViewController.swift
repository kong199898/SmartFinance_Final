...
class AddProfileFormViewController: UIViewController {
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if NameField.text != "" && BirthDateField.text != "" && SalaryField.text != ""{
            if checkName() && checkBirthdate() && checkSalary(){
                createItem(name: NameField.text!, birth: datePicker.date, earning: Int(SalaryField.text!) ?? 0)
    ...
    func createItem(name:String, birth:Date, earning: Int){
        let newitems = TheProfile(context: context)
        newitems.name = name
        newitems.birthdate = birth
        newitems.earning = Int64(earning)
        calculateAge(userBirth: birth)
