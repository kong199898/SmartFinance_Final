...
class SaleofPropertyDownsizeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editPropertyIncomeNotification), name: Notification.Name("editPropertyIncome"), object: nil)
    }
    
    @objc func editPropertyIncomeNotification(_ notification: Notification){
        isEdit = true
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                updateItem(item: editItem!, newamount: Int(AmountPerYearField.text!) ?? 0, newStartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }else{
                createItem(type: "Sale of Property/Downsize", amount: Int(AmountPerYearField.text!) ?? 0, StartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }
        }
    }
    ...
    func updateItem(item: TheIncomeEvent, newamount:Int, newStartingAge:Int){
