...
class RentalIncomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editRentalIncomeNotification), name: Notification.Name("editRentalIncome"), object: nil)
    }
    
    @objc func editRentalIncomeNotification(_ notification: Notification){
        isEdit = true
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                ...
                createItem(type: "Rental Income", amount: Int(AmountPerYearField.text!) ?? 0, frequency: frequency!, StartingAge: Int(AgeSlider.value))
                dismiss(animated: true, completion: nil)
            }
        }
    }
