...
class OtherIncomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(editOtherIncomeNotification), name: Notification.Name("editOtherIncome"), object: nil)
    }
    ...
    func validateInput() -> Bool{
        ...
        if DescriptionField.text == ""{
            showMessage(alert: "Invalid Description", message: "Please input a description of the income event.")
            return false
        }
    ...
    @objc func SubmitButtonTapped(_ sender: UIButton){
        if validateInput(){
            if isEdit!{
                ...
                createItem(type: "Other Income", amount: Int(AmountPerYearField.text!) ?? 0, frequency: frequency!, StartingAge: Int(AgeSlider.value), other: DescriptionField.text!)
                dismiss(animated: true, completion: nil)
            }
        }
    }
    ...
    @objc func editOtherIncomeNotification(_ notification: Notification){
        isEdit = true
    ...
    func updateItem(item: TheIncomeEvent, newamount:Int, newfrequency:String, newStartingAge:Int, newother:String){
